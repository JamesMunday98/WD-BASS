...
if sys.argv[1]=="ATM" or sys.argv[1]=="plotOnly":
	...
	if len(sys_args)==4 and sys.argv[2]=="run_double_one_by_oneMCMC":
		list_indexes=[]
	...
	while dowhile:
		...
		for aaaa in file_ignore:
			...
			for zcnt, aaa_sharerv in enumerate(original_share_rv):
				...
				if aaa_sharerv>=original_file_index and len(sys_args)==4 and sys.argv[2]=="run_double_one_by_oneMCMC":    list_indexes[zcnt]-=1
...
if starType1.startswith("D") and starType2.startswith("D"):
	...
	if sys.argv[1] == "ATM":
		...
		for cn, (fteff, flogg, fHHe, starType) in enumerate(zip([forced_teff1, forced_teff2], [forced_logg1, forced_logg2], [forced_HoverHe1, forced_HoverHe2], [starType1, starType2])):
			if fteff == 0 and flogg == 0:
				ndim+=2 # teff + logg
				p0range = np.array([p0T1, p0logg1, p0T2, p0logg2]);         p0labels = np.array(["T1", "logg1", "T2", "logg2"])
				pos_min = np.array([p0T1[0], p0logg1[0], p0T2[0], p0logg2[0]]);   pos_max = np.array([p0T1[1], p0logg1[1], p0T2[1], p0logg2[1]])
			elif fteff == 0:
				ndim+=1 # teff
				p0range = np.array([p0T1,p0T2]);         p0labels = np.array(["T1", "T2"])
				pos_min = np.array([p0T1[0], p0T2[0]]);   pos_max = np.array([p0T1[1], p0T2[1]])
			elif flogg == 0:
				ndim+=1 # logg
				p0range = np.array([p0logg1, p0logg2]);         p0labels = np.array(["logg", "logg2"])
				pos_min = np.array([p0logg1[0], p0logg2[0]]);   pos_max = np.array([p0logg1[1], p0logg2[1]])
	...
	if not (isinstance(forced_P0, float) and isinstance(forced_T0, float)):
		if forced_K1==False and forced_K2==False:
			...
			for i, (fi, arv, arvBounds2) in enumerate(zip(input_files, share_rv, RV_boundaries2)):
				if arv==-1:
					...
					# this is for corner plotting
					try:     p0range = np.concatenate((p0range, np.array([arvBounds2])))
		...
		if forced_K2=="Fit":
			...
			# this is for corner plotting
			try:     p0range = np.concatenate((p0range, np.array([p0K2])))
		...
		if forced_Vgamma2=="Fit":
			...
			# this is for corner plotting
			try:     p0range = np.concatenate((p0range, np.array([p0Vgamma2])))
...
## MCMC has been inititalised. Now loading the atmospheric grids if necessary
if starType1.startswith("D") or starType2.startswith("D"):
	...
	if pier_or_antoine=="pier3Dphot_antoine1Dspec":
	...
	elif (starType1=="DA" or starType2=="DA") and pier_or_antoine=="pier":
	...
	elif (starType1=="DA" or starType2=="DA") and pier_or_antoine=="antoine":
	...
	elif (starType1=="DA" or starType2=="DA") and pier_or_antoine=="mixed":
		...
		if False:
			if forced_teff1==0 and forced_teff2==0 and forced_logg1==0 and forced_logg2==0:
				...
				min_p0T = np.amin(np.array([p0T1[0], p0T1[1], p0T2[0], p0T2[1]]));    max_p0T = np.amax(np.array([p0T1[0], p0T1[1], p0T2[0], p0T2[1]]))
				...
				min_p0logg = np.amin(np.array([p0logg1[0], p0logg1[1], p0logg2[0], p0logg2[1]]));    max_p0logg = np.amax(np.array([p0logg1[0], p0logg1[1], p0logg2[0], p0logg2[1]]))
				...
				newMinLogg, newMaxLogg = np.amin(np.unique(Grav_all_synth)[mask_LOGG]), np.amax(np.unique(Grav_all_synth)[mask_LOGG])
	...
	if starType1=="DBA" or starType2=="DBA" or starType1=="DB" or starType2=="DB":
		...
		else:                    wl_all, flux_all, Teff_all, Grav_all, H_over_He_all = load_models.load_models_DBA(minwl=np.amin(reference_wl)-150,maxwl=np.amax(reference_wl)+250)
...
@njit
def gauss_if_no_flux_error(x, a, mu, sigma, b, mmm, ccc):
	return ((a/sigma/np.sqrt(2*np.pi)) * np.exp(-0.5 * np.square((x-mu)/sigma)) + b)  /  (x*mmm+ccc)
...
for files, normaliseHa, cut_Ha, ref_wl in zip(input_files, normaliseHa_all, cut_Ha_all, reference_wl):
	if spectra_source_type=="wl_flux_fluxerr":
		try:  wl_data, flux_data, flux_e_data = np.loadtxt(os.getcwd()+"/"+files, skiprows=1, unpack=True)  #  if you have flux errors, files are loaded in normally. Great.
		except:  
...
def return_DAgrids(temperature_star, logg_star):
	...
	if pier_or_antoine=="pier" or pier_or_antoine=="pier3Dphot_antoine1Dspec":
		if not (logg_star<7 and (temperature_star>14000 or temperature_star<5000)):   # 3D DA grid (unique structure for each of the below categories). Limits Logg = 5-9 . Teff 5000-14000
			...
			#plt.title(str(temperature_star) + "  " + str(logg_star));   plt.show();   plt.clf()
...
def return_DBAgrids(temperature_star, logg_star, HoverHestar):
	...
	Teff_min=unique_Teffs_DBA[np.argwhere(temdiff==np.amin(temdiff[temdiff>0]))[0][0]]
	Teff_max=unique_Teffs_DBA[np.argwhere(temdiff==np.amax(temdiff[temdiff<0]))[0][0]]
...
def lnprior(theta, arguments):
	...
	if "T2" in p0labels:            args = np.argwhere(p0labels=="T2")[0][0];          T2 = theta[args]
	else: T2=forced_teff2
	if "logg2" in p0labels:         args = np.argwhere(p0labels=="logg2")[0][0];       logg2 = theta[args]
	...
	if "H/He2" in p0labels:         args = np.argwhere(p0labels=="H/He2")[0][0];       HoverHe2 = theta[args]
	else:
		try: HoverHe2 = forced_HoverHe2
		except: None
	if "K1" in p0labels:            args = np.argwhere(p0labels=="K1")[0][0];          mcmc_K1 = theta[args]
	if "K2" in p0labels:            args = np.argwhere(p0labels=="K2")[0][0];          mcmc_K2 = theta[args]
	if "Vg1" in p0labels:           args = np.argwhere(p0labels=="Vg1")[0][0];         mcmc_Vgamma1 = theta[args]
	if "Vg2" in p0labels:           args = np.argwhere(p0labels=="Vg2")[0][0];         mcmc_Vgamma2 = theta[args]
	...
	if not (isinstance(forced_P0, float) and isinstance(forced_T0, float)):
		...
		if forced_K1=="Fit" and forced_K2=="Fit":
			if forced_Vgamma1=="Fit" and forced_Vgamma2=="Fit":
				...
				if not (p0K1[0] < mcmc_K1 < p0K1[1] and p0K2[0] < mcmc_K2 < p0K2[1]):              return -np.inf
		elif forced_K1=="Fit":
			if forced_Vgamma1=="Fit":
			...
			if forced_Vgamma2=="Fit":
				if not (p0K2[0] < mcmc_K2 < p0K2[1] and p0Vgamma2[0] < mcmc_Vgamma2 < p0Vgamma2[1]):    return -np.inf
			else:
				if not p0K2[0] < mcmc_K2 < p0K2[1]:       return -np.inf
		...
		if forced_K1!="Fit" and forced_Vgamma1=="Fit" and forced_K2!="Fit" and forced_Vgamma2=="Fit":
			if not (p0Vgamma1[0] < mcmc_Vgamma1 < p0Vgamma1[1] and p0Vgamma2[0] < mcmc_Vgamma2 < p0Vgamma2[1]):    return -np.inf
		if forced_K1!="Fit" and forced_Vgamma1!="Fit" and forced_K2!="Fit" and forced_Vgamma2=="Fit":
			if not p0Vgamma2[0] < mcmc_Vgamma2 < p0Vgamma2[1]:    return -np.inf
	...
	for ref_wl in the_unique_wavelengths:
		...
		for (ii, in_fi, sh_rv, modha, cut_lim, norm_lim, aHJD, sigclipspec) in zip(index[mask_ref_wl], input_files[mask_ref_wl], share_rv[mask_ref_wl], modelHa[mask_ref_wl], cut_Ha_all[mask_ref_wl], normaliseHa_all[mask_ref_wl], HJD_values[mask_ref_wl], sigma_clip[mask_ref_wl]):  # the mcmc RVs are gone through 1 by one, uses the ii counter.
			...
			if not (isinstance(forced_P0, float) and isinstance(forced_T0, float)):
				...
				if forced_K2=="Fit":   
					if forced_Vgamma2=="Fit":    mcmc_rv2 = mcmc_Vgamma2 + mcmc_K2*np.sin(2*np.pi*phi)
					else:    mcmc_rv2 = forced_Vgamma2 + mcmc_K2*np.sin(2*np.pi*phi)
				else:   
					if forced_Vgamma1=="Fit":    mcmc_rv2 = mcmc_Vgamma2 + forced_K2*np.sin(2*np.pi*phi)
					else:    mcmc_rv2 = forced_Vgamma2 + forced_K2*np.sin(2*np.pi*phi)
				if forced_K1!="Fit" and forced_Vgamma1=="Fit":     mcmc_rv1 = mcmc_Vgamma1 + forced_K1*np.sin(2*np.pi*phi)
				if forced_K2!="Fit" and forced_Vgamma2=="Fit":     mcmc_rv2 = mcmc_Vgamma2 + forced_K2*np.sin(2*np.pi*phi)
			...
			interparr = interp(model_wl2, model_wl1+dlam1, model_spectrum_star1)    + Scaling*interp(model_wl2, model_wl2+dlam2, model_spectrum_star2)
...
def lnprior_gauss_lorentz(theta, arguments):
	...
	args = np.argwhere(p0labels=="A2_1")[0][0];   gaussLorentz2_A1 = theta[args]
	args = np.argwhere(p0labels=="A2_2")[0][0];   gaussLorentz2_A2 = theta[args]
	...
	args = np.argwhere(p0labels=="sig2_1")[0][0];   gaussLorentz2_sigma1 = theta[args]
	args = np.argwhere(p0labels=="sig2_2")[0][0];   gaussLorentz2_sigma2 = theta[args]
...
def lnlike_gauss_lorentz(theta, arguments):
	...
	for ref_wl in the_unique_wavelengths:
		...
		model_wl2 = np.linspace(cut_wl_min, cut_wl_max, int(cut_wl_max-cut_wl_min)*10)
...
def lnlike_quad_lorentz(theta, arguments):
	...
	for ref_wl in the_unique_wavelengths:
		...
		for (ii, in_fi, sh_rv, modha, cut_lim, norm_lim, aHJD, sigclipspec) in zip(index[mask_ref_wl], input_files[mask_ref_wl], share_rv[mask_ref_wl], modelHa[mask_ref_wl], cut_Ha_all[mask_ref_wl], normaliseHa_all[mask_ref_wl], HJD_values[mask_ref_wl], sigma_clip[mask_ref_wl]):  # the mcmc RVs are gone through 1 by one, uses the ii counter.
			...
			quad = m4 * normalised_wavelength**4 + m3 * normalised_wavelength**3 + m2 * normalised_wavelength**2  +  m1*normalised_wavelength  +  c
...
if sys.argv[1]=="RV" or sys.argv[1]=="RV_gauss" or sys.argv[1]=="RV_double_commonRV12":
	...
	if starType1.startswith("D") or starType2.startswith("D"):
		...
		model_spectrum_star2*=Scaling_med
	
	
	else:
		model_wl1 = np.linspace(cut_limits_min, cut_limits_max, int(cut_limits_max-cut_limits_min)*10)
		model_wl2 = np.linspace(cut_limits_min, cut_limits_max, int(cut_limits_max-cut_limits_min)*10)
		...
		model_wl1, model_wl2 = model_wl1 + np.amax(reference_wl), model_wl2 + np.amax(reference_wl)
	...
	def residual(params, xxx, yyy, yyyerr,wl1,spec1,wl2,spec2,gauss1,gauss2):
	...
	def fit_bootstrap(params, xxx, yyy, yyyerr, bounds, num_its, wl1, spec1, wl2, spec2, gauss1=np.array([1,1]), gauss2=np.array([1,1]), Common_RV_both_stars=False):
		...
		RV1_guess, RV2_guess, extra_norm_guess, coff_guess = params
		RV1, RV2, offset, extra_norm, coff = [], [], [], [], []
		...
		if Common_RV_both_stars==False:
			for i in range(num_its):
				...
				lsm_fit = least_squares(residual, [RV1_guess, RV2_guess, extra_norm_guess, coff_guess], method='trf',
				                args=(x,y,err,wl1,spec1,wl2,spec2,gauss1,gauss2), ftol=1E-10, bounds=((bounds[0], bounds[2], bounds[4], bounds[6]), (bounds[1], bounds[3], bounds[5], bounds[7])))
			...
			for i in range(num_its):
				...
				lsm_fit = least_squares(residual_commonRV, [RV1_guess, extra_norm_guess, coff_guess], method='trf',
				                args=(x,y,err,wl1,spec1,wl2,spec2,gauss1,gauss2), ftol=1E-10, bounds=((bounds[0], bounds[4], bounds[6]), (bounds[1], bounds[5], bounds[7])))
	...
	def fit_bootstrap2(params, xxx, yyy, yyyerr, bounds, num_its, wl1, spec1, wl2, spec2, gauss1=np.array([1,1]), gauss2=np.array([1,1]), Common_RV_both_stars=False, model_range=40):
		'''Samples the data a thousand times, fits each sample using least-squares,
		then reports median and standard deviation of the result'''
		...
		if Common_RV_both_stars==False:
			for i in range(num_its):
				...
				lsm_fit = least_squares(residual2, [RV1_guess, RV2_guess, off_guess], method='trf',
				                args=(x,y,err,wl1,spec1,wl2,spec2,gauss1,gauss2, model_range), ftol=1E-10, bounds=((bounds[0], bounds[2], bounds[4]), (bounds[1], bounds[3], bounds[5])))
			...
			for i in range(num_its):
				...
				lsm_fit = least_squares(residual2_commonRV, [RV1_guess, off_guess], method='trf',
				                args=(x,y,err,wl1,spec1,wl2,spec2,gauss1,gauss2), ftol=1E-10, bounds=((bounds[0], bounds[4]), (bounds[1], bounds[5])))
	...
	for ii, (in_fi, modha, cut_lim, norm_lim, aHJD, sigclipspec) in enumerate(zip(input_files[mask_ref_wl], modelHa[mask_ref_wl], cut_Ha_all[mask_ref_wl], normaliseHa_all[mask_ref_wl], HJD_values[mask_ref_wl], sigma_clip[mask_ref_wl])):
		...
		if np.amin(modha)>-40 or np.amax(modha)<40:   model_range = np.amin(np.abs(modha))
		else:   model_range=np.amin(np.abs(modha))
		...
		try:
			...
			if starType1=="quadLorentz" and starType2=="quadLorentz":
				...
				smear_model_spectrum_star2_temp = smear_model_spectrum_star2 + 1 - 1
				m4, m3, m2, m1, c = polyfit((normalised_wavelength-ref_wl), normalised_flux - interp(normalised_wavelength, model_wl1, smear_model_spectrum_star1 - smear_model_spectrum_star2), 4)
			...
			if np.amin(modha)>-40 or np.amax(modha)<40: Low_SNR=False
			RV1, RV2 = np.mean(rvbound1), np.mean(rvbound2)   # this is the initial guess for the bootstrap
			if Low_SNR:
				# First try to improve the accuracy of the normalisation using the model and the full wavelength range supplied
				if sys.argv[1]=="RV_gauss":
					returnvars = fit_bootstrap([RV1, RV2, 0, 1],  normalised_wavelength,  normalised_flux,  normalised_err, bounds=[rvbound1[0], rvbound1[1], rvbound2[0], rvbound2[1], -0.02,0.02, -20, 20], num_its=25, wl1=model_wl1, spec1=smear_model_spectrum_star1, wl2=model_wl2, spec2=smear_model_spectrum_star2, gauss1=smear_gauss1, gauss2=smear_gauss2, Common_RV_both_stars=commonRV_1_2)
				else:
					
					returnvars = fit_bootstrap([RV1, RV2, 0, 1],  normalised_wavelength,  normalised_flux,  normalised_err, bounds=[rvbound1[0], rvbound1[1], rvbound2[0], rvbound2[1], -0.02,0.02, -20, 20], num_its=25, wl1=model_wl1, spec1=smear_model_spectrum_star1, wl2=model_wl2, spec2=smear_model_spectrum_star2, Common_RV_both_stars=commonRV_1_2)
				try: RV1, RV1err, RV2, RV2err, extra_norm, extra_norm_err, coff, coff_err = returnvars
				...
				if sys.argv[1]=="RV_gauss":
					returnvars = fit_bootstrap([RV1, RV2, extra_norm, coff],  normalised_wavelength[clip_mask],  normalised_flux[clip_mask],  normalised_err[clip_mask], bounds=[rvbound1[0], rvbound1[1], rvbound2[0], rvbound2[1], -0.02,0.02, -20, 20], num_its=100, wl1=model_wl1, spec1=smear_model_spectrum_star1, wl2=model_wl2, spec2=smear_model_spectrum_star2, gauss1=smear_gauss1, gauss2=smear_gauss2, Common_RV_both_stars=commonRV_1_2)
				else:
					try:
						...
						issue_here=True
						returnvars = fit_bootstrap([RV1, RV2, extra_norm, coff],  normalised_wavelength,  normalised_flux,  normalised_err, bounds=[rvbound1[0], rvbound1[1], rvbound2[0], rvbound2[1], -20.0,20.0, -20, 20], num_its=100, wl1=model_wl1, spec1=smear_model_spectrum_star1, wl2=model_wl2, spec2=smear_model_spectrum_star2, Common_RV_both_stars=commonRV_1_2)
			...
			if sigclipspec!=-1 and not (starType1=="quadLorentz" and starType2=="quadLorentz"):
			...
			if sigclipspec!=-1:
				...
				if not issue_here:
					returnvars = fit_bootstrap2([RV1, RV2, 0],  normalised_wavelength[clip_mask],  normalised_flux[clip_mask],  normalised_err[clip_mask], bounds=[rvbound1[0], rvbound1[1], rvbound2[0], rvbound2[1], -0.2, 0.2], num_its=50, wl1=model_wl1, spec1=smear_model_spectrum_star1, wl2=model_wl2, spec2=smear_model_spectrum_star2, model_range=model_range, Common_RV_both_stars=commonRV_1_2)
				else:
					returnvars = fit_bootstrap2([RV1, RV2, 0],  normalised_wavelength,  normalised_flux,  normalised_err, bounds=[rvbound1[0], rvbound1[1], rvbound2[0], rvbound2[1], -20.0, 20.0], num_its=50, wl1=model_wl1, spec1=smear_model_spectrum_star1, wl2=model_wl2, spec2=smear_model_spectrum_star2, model_range=model_range, Common_RV_both_stars=commonRV_1_2)
				...
				mask_narrow = ((normalised_wavelength > desired_refwl-40) & (normalised_wavelength < desired_refwl+40)) | ((normalised_wavelength<desired_refwl+norm_limits_min+5) | (normalised_wavelength>desired_refwl+norm_limits_max-5))
				...
				if starType1=="quadLorentz" and starType2=="quadLorentz":
					m4, m3, m2, m1, c = polyfit((normalised_wavelength[mask_narrow][clip_mask2]-ref_wl), normalised_flux[mask_narrow][clip_mask2] - interp(normalised_wavelength[mask_narrow][clip_mask2], model_wl1, smear_model_spectrum_star1 - smear_model_spectrum_star2), 4)
					quad = m4 * (model_wl1-ref_wl)**4  +  m3 * (model_wl1-ref_wl)**3  +  m2 * (model_wl1-ref_wl)**2  +  m1*(model_wl1-ref_wl)  +  c
					smear_model_spectrum_star1= smear_model_spectrum_star1_temp + 0.5*quad
					smear_model_spectrum_star2 = smear_model_spectrum_star2_temp + 0.5*quad
			...
			if not issue_here:
				returnvars = fit_bootstrap2([RV1, RV2, off],  normalised_wavelength[mask_narrow][clip_mask2],  normalised_flux[mask_narrow][clip_mask2],  normalised_err[mask_narrow][clip_mask2], bounds=[rvbound1[0], rvbound1[1], rvbound2[0], rvbound2[1], -0.2, 0.2], num_its=200, wl1=model_wl1, spec1=smear_model_spectrum_star1, wl2=model_wl2, spec2=smear_model_spectrum_star2, model_range=model_range, Common_RV_both_stars=commonRV_1_2)
			else:
	...
	if starType1.startswith("D") or starType2.startswith("D"):
		if forced_teff1!=0 and forced_logg1!=0 and forced_teff2!=0 and forced_logg2!=0 and starType1=="DA" and starType2=="DA":
			...
			sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, pool=pool, args=[[input_files, share_rv, reference_wl, cut_Ha_all, normaliseHa_all, list_norm_wl_grids, list_normalised_flux, list_normalised_err,  resolutions, used_RV_boundaries, HJD_values, sigma_clip, np.unique(reference_wl), model_wl1, model_spectrum_star1, model_wl2, model_spectrum_star2, inputScaling]])
			
			
		else:
			sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, pool=pool, args=[[input_files, share_rv, reference_wl, cut_Ha_all, normaliseHa_all, list_norm_wl_grids, list_normalised_flux, list_normalised_err,  resolutions, used_RV_boundaries, HJD_values, sigma_clip, np.unique(reference_wl), None, None, None, None, None]])
	
	elif starType1=="quadLorentz" and starType2=="quadLorentz":
		sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob_quad_lorentz, pool=pool, args=[[input_files, share_rv, reference_wl, cut_Ha_all, normaliseHa_all, list_norm_wl_grids, list_normalised_flux, list_normalised_err,  resolutions, used_RV_boundaries, HJD_values, sigma_clip, np.unique(reference_wl), None, None, None, None, None]])
	
	else:
		sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob_gauss_lorentz, pool=pool, args=[[input_files, share_rv, reference_wl, cut_Ha_all, normaliseHa_all, list_norm_wl_grids, list_normalised_flux, list_normalised_err,  resolutions, used_RV_boundaries, HJD_values, sigma_clip, np.unique(reference_wl), None, None, None, None, None]])
	...
	if "T2" in p0labels:
		args = np.argwhere(p0labels=="T2")[0][0];          Teff2_result = samples[::,args]
	...
	if "logg2" in p0labels:
		args = np.argwhere(p0labels=="logg2")[0][0];       logg2_result = samples[::,args]
	...
	if "H/He2" in p0labels:
		args = np.argwhere(p0labels=="H/He2")[0][0];       HoverHe2_result = samples[::,args]
	...
	if "K2" in p0labels:
		args = np.argwhere(p0labels=="K2")[0][0];          K2_result = samples[::,args]
	...
	if "Vg2" in p0labels:
		args = np.argwhere(p0labels=="Vg2")[0][0];         Vgamma2_result = samples[::,args]
	...
	if "A2_1" in p0labels:
		args = np.argwhere(p0labels=="A2_1")[0][0];         A2_1_result = samples[::,args]
	...
	if "sig2_1" in p0labels:
		args = np.argwhere(p0labels=="sig2_1")[0][0];         sig2_1_result = samples[::,args]
	...
	if "A2_2" in p0labels:
		args = np.argwhere(p0labels=="A2_2")[0][0];         A2_2_result = samples[::,args]
	...
	if "sig2_2" in p0labels:
		args = np.argwhere(p0labels=="sig2_2")[0][0];         sig2_2_result = samples[::,args]
	...
	if "RV1_0" in p0labels:
		...
		for aHJD in HJD_values:
			...
			allRV2s.append(Vgamma2_med + K2_med*np.sin(2*np.pi*phi));   allRV2s_minerr.append(0);   allRV2s_maxerr.append(0)
	...
	for ii, (in_fi, sh_rv, ref_wl, modha, cut_lim, norm_lim, mask_out_Ha_min, mask_out_Ha_max, aHJD, inp_resolution, sigclipspec) in enumerate(zip(input_files, share_rv, reference_wl, modelHa, cut_Ha_all, normaliseHa_all, mask_out_Ha_min_all, mask_out_Ha_max_all, HJD_values, resolutions, sigma_clip)):
		if ((starType1.startswith("GG") or starType1.startswith("LL") or starType1.startswith("GL")) and sh_rv==-1) or (sh_rv==-1 and not ii in share_rv):
			...
			if starType1.startswith("D") and starType2.startswith("D"):
				...
				model_wl2 = np.linspace(cut_limits_min, cut_limits_max, int(cut_limits_max-cut_limits_min)*10) #+ ref_wl
			...
			if starType1=="quadLorentz" and starType2=="quadLorentz":
				...
				quad = m4 * (normalised_wavelength-ref_wl)**4  +  m3 * (normalised_wavelength-ref_wl)**3  +  m2 * (normalised_wavelength-ref_wl)**2  +  m1*(normalised_wavelength-ref_wl)  +  c
				interparr += quad
			...
			#plt.plot(normalised_wavelength, interparr);   plt.plot(np.linspace(6000, 7000, 1000), m*np.linspace(6000, 7000, 1000) + c);  plt.show()
			interparr /= (m*normalised_wavelength + c)
			...
			if ref_wl>6000:
				np.savetxt("out/fit_"+in_fi+".dat", np.array([normalised_wavelength[desired_range] - ref_wl, interparr[desired_range]]).T)
				np.savetxt("out/data_for_fit_"+in_fi+".dat", np.array([normalised_wavelength - ref_wl, normalised_flux]).T)
	...
	for unique_shared_rv in np.unique(share_rv[where_share_rv]):
		...
		for cn_shared, (in_fi, ref_wl, modha, cut_lim, norm_lim, normalised_wavelength, normalised_flux, normalised_err, inp_resolution, mask_out_Ha_min, mask_out_Ha_max, sigclipspec) in enumerate(zip(input_files_combinations, reference_wl_combinations, modelHa_combinations, cut_Ha_combinations, normaliseHa_combinations, norm_wl_combinations, normalised_flux_combinations, normalised_err_combinations, resolutions_combinations, mask_out_Ha_min_all_combinations, mask_out_Ha_max_all_combinations, sigma_clip_combinations)):
			...
			if False:
				...
				interparr = interparr1 + Scaling_med*interparr2
			...
			if plot_fancy==False:
				...
				textvalmin, textvalmax = np.amin(mins), np.amax(maxes)
				difference = textvalmax - textvalmin
				
				
				if np.round(ref_wl,0)==6563:   toappendtext.append([np.amin(mins)+difference/25, np.amax(interparr+offset)-0.15, r"H$\alpha$", np.amin(mins), np.amax(maxes)])
				elif np.round(ref_wl,0)==4861: toappendtext.append([np.amin(mins)+difference/25,np.amax(interparr+offset)-0.1,r"H$\beta$", np.amin(mins), np.amax(maxes)])
				elif np.round(ref_wl,0)==4340: toappendtext.append([np.amin(mins)+difference/25,np.amax(interparr+offset)-0.075,r"H$\gamma$", np.amin(mins), np.amax(maxes)])
				elif np.round(ref_wl,0)==4102: toappendtext.append([np.amin(mins)+difference/25,np.amax(interparr+offset)-0.05-cn_shared*0.01,r"H$\delta$", np.amin(mins), np.amax(maxes)])
				elif np.round(ref_wl,0)==3970: toappendtext.append([np.amin(mins)+difference/25,np.amax(interparr+offset)-0.045-cn_shared*0.01,r"H$\epsilon$", np.amin(mins), np.amax(maxes)])
				elif np.round(ref_wl,0)==3889: toappendtext.append([np.amin(mins)+difference/25,np.amax(interparr+offset)-0.025-cn_shared*0.01,r"H$\zeta$", np.amin(mins), np.amax(maxes)])
				elif np.round(ref_wl,0)==3835: toappendtext.append([np.amin(mins)+difference/25,np.amax(interparr+offset)+0.02-cn_shared*0.01,r"H$\eta$", np.amin(mins), np.amax(mins)])
				else: name="UNDEF"
			...
			if plot_fancy==False:
				...
				if np.amin(interparr[desired_range] + offset) < minstored:
					minstored=np.amin(interparr[desired_range] + offset)
				if np.amin(normalised_flux[mask_model] + offset) < minstored:
					minstored=np.amin(normalised_flux[mask_model] + offset)
				if np.amax(interparr[desired_range] + offset) > maxstored:
					maxstored=np.amax(interparr[desired_range] + offset)
				if np.amax(normalised_flux[mask_model] + offset) > maxstored:
					maxstored=np.amax(normalised_flux[mask_model] + offset)
		...
		if plot_fancy==False:
			...
			ax.scatter(0,1,alpha=0, label="refwave = " + str(ref_wl))
			...
			ax2.scatter(0,1,alpha=0, label="refwave = " + str(ref_wl))
			...
			ax.set_xlim(np.amin(xmin), np.amax(xmax))
			...
			textvalmin, textvalmax = np.amin(xmin), np.amax(xmax)
