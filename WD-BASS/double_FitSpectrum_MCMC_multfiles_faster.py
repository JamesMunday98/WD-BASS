...
if forced_ephemeris!=False:
    ...
    p0K2 = np.array(config_info["K2_boundaries"], dtype=float)
    ...
    p0Vgamma2 = np.array(config_info["Vgamma2_boundaries"], dtype=float)
else:
...
except: force_CO_MTR=np.array([False, False])
try: high_RV_amp=bool(np.asarray([config_info["high_RV_amp"]])[0])
except: high_RV_amp=False
try: continuum_normalisation=np.asarray(config_info["continuum_normalisation"]).astype(bool)
except: continuum_normalisation=True
try: input_vacuum_wl=np.asarray(config_info["input_vacuum_wl"]).astype(bool)
except: input_vacuum_wl=False
...
if "run_double_one_by_oneMCMC" in sys_args:
...
if sys_args[1]=="ATM" or sys_args[1]=="plotOnly":

    
    if "run_double_one_by_oneMCMC" in sys_args:   file_ignore=np.array([])
    ...
    while dowhile:
        ...
        for aaaa in file_ignore:
            ...
            for zcnt, aaa_sharerv in enumerate(original_share_rv):
                ...
                if aaa_sharerv>=original_file_index and "run_double_one_by_oneMCMC" in sys_args:    list_indexes[zcnt]-=1
...
if forced_ephemeris!=False and not (forced_K1=="Fit" or isinstance(forced_K1, float) or forced_Vgamma1=="Fit" or isinstance(forced_Vgamma1, float)  or  forced_K2=="Fit" or isinstance(forced_K2, float) or forced_Vgamma2=="Fit" or isinstance(forced_Vgamma2, float)):  raise ValueError("Error - knowing the ephemeris and not fitting or knowing any K or Vgamma is weird. Not allowed")
...
if (starType1.startswith("sd") or starType1=="ELM") and fit_phot_SED:
    want_R=True
...
if starType1.startswith("D") or starType2.startswith("D") or starType1=="ELM" or starType2=="ELM":

    pier_or_antoine="mixed"#"pier3Dphot_antoine1Dspec"

    if (starType1=="DA" or starType2=="DA") and pier_or_antoine=="mixed":
        if fit_phot_SED or continuum_normalisation==False:         wl_all_synth, flux_all_synth, Teff_all_synth, Grav_all_synth = load_models.load_models_DA_3D_NLTE(minwl=theminww_loadgrid,maxwl=themaxww_loadgrid)
        ...
        if False:
            if forced_teff1==0 and forced_teff2==0 and forced_logg1==0 and forced_logg2==0:
                ...
                newMinLogg, newMaxLogg = npamin(unique_Grav_all_synth[mask_LOGG]), npamax(unique_Grav_all_synth[mask_LOGG])
...
@njit
def linear_fit_weighted(x, y, yerr):
    """
    Weighted linear regression (handles y errors).
    ...
    y : 1D array
        Dependent variable
    yerr : 1D array
        Errors on y (must be > 0)
    """
    w = 1.0 / (yerr * yerr)  # weights
...
for files, normaliseHa, cut_Ha, ref_wl in zip(input_files, normaliseHa_all, cut_Ha_all, reference_wl):
    if spectra_source_type=="wl_flux_fluxerr":
        try:  
            if JM_environment and ((files.startswith("DESI_") or (files.startswith("SDSS")) and not "COADD" in files)):
                #wl_data, flux_data, ivar_data = nploadtxt(getcwd+"/"+files, skiprows=1, usecols=(0,1,2),unpack=True)  #  if you have flux errors, files are loaded in normally. Great.
                wl_data, flux_data, ivar_data, bitmask = nploadtxt(getcwd+"/"+files, skiprows=1,unpack=True)  #  if you have flux errors, files are loaded in normally. Great.
                
                line1 = open(getcwd+"/"+files).readline()
        ...
        if input_vacuum_wl==True: # convert to air  https://www.astro.uu.se/valdwiki/Air-to-vacuum%20conversion
            s = 10**4/wl_data
            wl_data = wl_data / (1 + 0.0000834254 + 0.02406147 / (130 - s**2) + 0.00015998 / (38.9 - s**2))
        ...
        if continuum_normalisation:
            ...
            try:
                while num_its<100:
                    ...
                    m,c = linear_fit_weighted(wl_data_normmask[good], flux_data_normmask[good], fluxe_data_normmask[good])
                ...
                m,c = linear_fit_weighted(wl_data_normmask, flux_data_normmask, fluxe_data_normmask)
            ...
            normalised_wl_grid = wl_data;    normalised_flux = flux_data;    normalised_err = flux_e_data
            mask_out_Ha_min_all.append(ref_wl+normaliseHa[0]);    mask_out_Ha_max_all.append(ref_wl+normaliseHa[1])
...
if continuum_normalisation==False:
    min_cutHa, max_cutHa = float(np.amin(list_norm_wl_grids)-ref_wl), float(np.amax(list_norm_wl_grids)-ref_wl)
    for i in range(len(cut_Ha_all)):
        cut_Ha_all[i][0]=min_cutHa
        cut_Ha_all[i][1]=max_cutHa
    

if "run_double_one_by_oneMCMC" in sys_args and (starType1=="quadLorentz" and starType2=="quadLorentz"):
...
if (starType1.startswith("D") and starType2.startswith("D")) or (starType1.startswith("ELM") and starType2.startswith("ELM")):
    ...
    if sys_args[1] == "ATM" and not "run_double_one_by_oneMCMC" in sys_args:
        ndim+=8
...
@njit
def return_DAgrids_ORIGINAL(temperature_star, logg_star):
...
if True: ## claude speedup
    ...
    def build_DA_grid_lookup():
        lookup, lookup_norm_spec = {}, {}
        logg_grid = np.array([6.5, 7.0, 7.5, 8.0, 8.5, 9.0])
        
        mask_wl_grid_for_normspec = wl_all_synth != wl_all_synth
        for ref_wl in npunique(reference_wl):
            ...
            mask_wl_grid_for_normspec = mask_wl_grid_for_normspec | ((wl_all_synth>ref_wl + cut_wl_min - 10) & (wl_all_synth<ref_wl + cut_wl_max + 10))
        ...
        for Tmin, Tmax in teff_bracket_list:
            for gmin, gmax in logg_bracket_list:
                ...
                lookup_norm_spec[(Tmin, Tmax, gmin, gmax)] = (
                    logg_all_synth[mask & mask_wl_grid_for_normspec],
                    wl_all_synth[mask & mask_wl_grid_for_normspec],
                    flux_all_synth[mask & mask_wl_grid_for_normspec],
                    Teff_all_synth[mask & mask_wl_grid_for_normspec],
                )
        return lookup, lookup_norm_spec
    ...
    def return_DAgrids_normspec(temperature_star, logg_star):
        Teff_min, Teff_max, minval, maxval = _find_DA_bracket(temperature_star, logg_star)
        return DA_GRID_LOOKUP_NORMSPEC[(Teff_min, Teff_max, minval, maxval)]
    ...
    DA_GRID_LOOKUP, DA_GRID_LOOKUP_NORMSPEC = build_DA_grid_lookup()
...
if starType1=="DB" or starType2=="DB":
    @njit
...
@njit
def return_model_spectrum_DA(wl_all1_N, ref_wl, cut_limits_min, cut_limits_max, Grav1_N, flux1_N, Teff1_N, temperature_star, logg_star, Ts_Gs_unique=None):
    ...
    Grav_N_N = Grav1_N[mask_logg_wl];   flux_N_N=flux1_N[mask_logg_wl];    Teff_N_N=Teff1_N[mask_logg_wl]  #  ; wl_all_N_N=wl_all1_N[mask_logg_wl]
    
    wl_grid = npunique(wl_all1_N[mask_logg_wl])
    ...
    #except:    raise ValueError(unique_Ts, unique_Gs, unique_Ts[0], unique_Ts[1], unique_Gs[0], unique_Gs[1], len(flux_N_N[maskT0 & maskG0]), len(flux_N_N[maskT0 & maskG1]), len(flux_N_N[maskT1 & maskG0]), len(flux_N_N[maskT1 & maskG1]), tereturn_model_spectrum_DAmperature_star, logg_star)
    
    
    
    if Ts_Gs_unique is not None:
        T0, T1, G0, G1 = Ts_Gs_unique
    else:
    ...
    delta_wl_max_min = wl_grid[-1] - wl_grid[0]  # because I did npunique above, the wl_grid is ordered, so the first element is the min and the last element is the max 
    
    if ref_wl>6500:     fine_grid=nplinspace(wl_grid[0],wl_grid[-1],int((delta_wl_max_min)*50)) # 0.02AA spacing
    elif ref_wl>4500:   fine_grid=nplinspace(wl_grid[0],wl_grid[-1],int((delta_wl_max_min)*33)) # 0.03AA spacing
    elif ref_wl>4200:   fine_grid=nplinspace(wl_grid[0],wl_grid[-1],int((delta_wl_max_min)*20)) # 0.05AA spacing
    else:               fine_grid=nplinspace(wl_grid[0],wl_grid[-1],int((delta_wl_max_min)*10)) # 0.1AA spacing
    
    
    return fine_grid, interp(fine_grid, wl_grid, model_spectrum)
    

if starType1=="DBA" or starType2=="DBA":
    #@njit
    ...
    def return_model_spectrum_DBA(wl_all1_N, ref_wl, cut_limits_min, cut_limits_max, Grav1_N, flux1_N, Teff1_N, HoverHe1_N, temperature_star, logg_star, HoverHe_star):
        ...
        if ref_wl>6500:     fine_grid=nplinspace(wl_grid[0],wl_grid[-1],int((wl_grid[-1] - wl_grid[0])*10)) # 0.1AA spacing
        elif ref_wl>4500:   fine_grid=nplinspace(wl_grid[0],wl_grid[-1],int((wl_grid[-1] - wl_grid[0])*10)) # 0.1AA spacing
        elif ref_wl>4200:   fine_grid=nplinspace(wl_grid[0],wl_grid[-1],int((wl_grid[-1] - wl_grid[0])*10)) # 0.1AA spacing
        else:               fine_grid=nplinspace(wl_grid[0],wl_grid[-1],int((wl_grid[-1] - wl_grid[0])*10)) # 0.1AA spacing
...
PARAM_IDX = {
    ...
    "K2":        param_index.get("K2"),
    "Vg1":       param_index.get("Vg1"),
    "Vg2":       param_index.get("Vg2"),
...
IDX_T1        = PARAM_IDX["T1"];     IDX_logg1     = PARAM_IDX["logg1"];   IDX_T2        = PARAM_IDX["T2"];     IDX_logg2     = PARAM_IDX["logg2"]
IDX_HoverHe1  = PARAM_IDX["H/He1"];  IDX_HoverHe2  = PARAM_IDX["H/He2"];
IDX_K1        = PARAM_IDX["K1"];     IDX_K2        = PARAM_IDX["K2"];      IDX_Vg1       = PARAM_IDX["Vg1"];    IDX_Vg2       = PARAM_IDX["Vg2"]
IDX_Scaling   = PARAM_IDX["Scaling"]; IDX_Parallax  = PARAM_IDX["Parallax"];  IDX_R         = PARAM_IDX["R"]
IDX_Dummy     = PARAM_IDX["Dummy"];  IDX_BBT       = PARAM_IDX["BBT"];     IDX_BBR       = PARAM_IDX["BBR"]

if IDX_T1 is None: IDX_T1=-1
if IDX_T2 is None: IDX_T2=-1
if IDX_logg1 is None: IDX_logg1=-1
if IDX_logg2 is None: IDX_logg2=-1
if IDX_HoverHe1 is None: IDX_HoverHe1=-1
if IDX_HoverHe2 is None: IDX_HoverHe2=-1
if IDX_K1 is None: IDX_K1=-1
if IDX_K2 is None: IDX_K2=-1
if IDX_Vg1 is None: IDX_Vg1=-1
if IDX_Vg2 is None: IDX_Vg2=-1
...
def lnprior(theta, arguments):
    #_, _, _, _, _, _, _, _, _, used_RV_boundaries, _, _, _, _, _, _, _, _ = arguments
    ...
    #T2 = theta[param_index["T2"]] if "T2" in param_index else forced_teff2
    #logg2 = theta[param_index["logg2"]] if "logg2" in param_index else forced_logg2
    #HoverHe1 = theta[param_index["H/He1"]] if "H/He1" in param_index else forced_HoverHe1
    #HoverHe2 = theta[param_index["H/He2"]] if "H/He2" in param_index else forced_HoverHe2
    #mcmc_K1 = theta[param_index["K1"]] if "K1" in param_index else None
    #mcmc_K2 = theta[param_index["K2"]] if "K2" in param_index else None
    #mcmc_Vgamma1 = theta[param_index["Vg1"]] if "Vg1" in param_index else None
    #mcmc_Vgamma2 = theta[param_index["Vg2"]] if "Vg2" in param_index else None
    ...
    if IDX_T2>=0 and not (p0T2[0]<theta[IDX_T2]<p0T2[1]): return -np.inf
    if IDX_logg1>=0 and not (p0logg1[0]<theta[IDX_logg1]<p0logg1[1]): return -np.inf
    if IDX_logg2>=0 and not (p0logg2[0]<theta[IDX_logg2]<p0logg2[1]): return -np.inf
    if IDX_HoverHe1>=0 and not (p0HoverHe1[0]<theta[IDX_HoverHe1]<p0HoverHe1[1]): return -np.inf
    if IDX_HoverHe2>=0 and not (p0HoverHe2[0]<theta[IDX_HoverHe2]<p0HoverHe2[1]): return -np.inf
    ...
    #if checkT2 and not p0T2[0]<T2<p0T2[1]: passed = False
    #if checklogg2 and not p0logg2[0]<logg2<p0logg2[1]: passed = False
    #if checkHoverHe1 and not p0HoverHe1[0]<HoverHe1<p0HoverHe1[1]: passed = False
    #if checkHoverHe2 and not p0HoverHe2[0]<HoverHe2<p0HoverHe2[1]: passed = False
    ...
    if not (isinstance(forced_P0, float) and isinstance(forced_T0, float)):
        ...
        if IDX_Vg2>=0: mcmc_Vgamma2 = theta[IDX_Vg2]
        if IDX_K2>=0: mcmc_K2 = theta[IDX_K2]
        ...
        if forced_K1=="Fit" and forced_K2=="Fit":
            if forced_Vgamma1=="Fit" and forced_Vgamma2=="Fit":
                ...
                if not (p0K1[0] < mcmc_K1 < p0K1[1] and p0K2[0] < mcmc_K2 < p0K2[1]):              return minus_npinf
        elif forced_K1=="Fit":
            if forced_Vgamma1=="Fit":
            ...
            if forced_Vgamma2=="Fit":
                if not (p0K2[0] < mcmc_K2 < p0K2[1] and p0Vgamma2[0] < mcmc_Vgamma2 < p0Vgamma2[1]):    return minus_npinf
            else:
                if not p0K2[0] < mcmc_K2 < p0K2[1]:       return minus_npinf
        ...
        if forced_K1!="Fit" and forced_Vgamma1=="Fit" and forced_K2!="Fit" and forced_Vgamma2=="Fit":
            if not (p0Vgamma1[0] < mcmc_Vgamma1 < p0Vgamma1[1] and p0Vgamma2[0] < mcmc_Vgamma2 < p0Vgamma2[1]):    return minus_npinf
        if forced_K1!="Fit" and forced_Vgamma1!="Fit" and forced_K2!="Fit" and forced_Vgamma2=="Fit":
            if not p0Vgamma2[0] < mcmc_Vgamma2 < p0Vgamma2[1]:    return minus_npinf
    
    if IDX_Parallax>=0:
        return np.log(1.0/(npsqrt(2*np_pi)*plax_unc))-0.5*(theta[IDX_Parallax]-plax)**2/plax_unc**2
    return 0.0
...
def lnlike(theta, arguments):
    ...
    T1 = theta[IDX_T1] if IDX_T1>=0 else forced_teff1
    logg1 = theta[IDX_logg1] if IDX_logg1>=0 else forced_logg1
    T2 = theta[IDX_T2] if IDX_T2>=0 else forced_teff2
    logg2 = theta[IDX_logg2] if IDX_logg2>=0 else forced_logg2
    HoverHe1 = theta[IDX_HoverHe1] if IDX_HoverHe1>=0 else forced_HoverHe1
    HoverHe2 = theta[IDX_HoverHe2] if IDX_HoverHe2>=0 else forced_HoverHe2
    mcmc_K1 = theta[IDX_K1] if IDX_K1>=0 else forced_K1
    mcmc_K2 = theta[IDX_K2] if IDX_K2>=0 else forced_K2
    mcmc_Vgamma1 = theta[IDX_Vg1] if IDX_Vg1>=0 else forced_Vgamma1
    mcmc_Vgamma2 = theta[IDX_Vg2] if IDX_Vg2>=0 else forced_Vgamma2
    Scaling = theta[IDX_Scaling] if IDX_Scaling>=0 else forced_Scaling
    mcmc_parallax = theta[IDX_Parallax] if IDX_Parallax>=0 else None
    #mcmc_R = theta[param_index["R"]] if "R" in param_index else None
    #mcmc_Dummy = theta[param_index["Dummy"]] if "Dummy" in param_index else None
    #mcmc_BBT = theta[param_index["BBT"]] if "BBT" in param_index else None
    #mcmc_BBR = theta[param_index["BBR"]] if "BBR" in param_index else None
    ...
    if fit_phot_SED:
        ...
        no_need_to_recompute = smeared_wl[0] < the_unique_wavelengths[0]+npamin(cut_Ha_all.T) - 10 and smeared_wl[-1] > the_unique_wavelengths[-1]+npamax(cut_Ha_all.T) + 10
    else:   no_need_to_recompute = False
    
    
    
    if starType1=="DA":
        _T0, _T1 = two_unique_values(Teff1_N)
        _G0, _G1 = two_unique_values(Grav1_N)
        star1_Ts_Gs = [_T0, _T1, _G0, _G1]
        if not no_need_to_recompute: # here I make the grid smaller so that only the areas covered by the normalised spectral lines (with 10A excess for each line) is considered
            Grav1_N, wl_all1_N, flux1_N, Teff1_N = return_DAgrids_normspec(T1, logg1)
    if starType2=="DA":
        _T0, _T1 = two_unique_values(Teff2_N)
        _G0, _G1 = two_unique_values(Grav2_N)
        star2_Ts_Gs = [_T0, _T1, _G0, _G1]
        if not no_need_to_recompute: # here I make the grid smaller so that only the areas covered by the normalised spectral lines (with 10A excess for each line) is considered
            Grav2_N, wl_all2_N, flux2_N, Teff2_N = return_DAgrids_normspec(T2, logg2)
    ...
    for ref_wl in the_unique_wavelengths:
        ...
        if no_need_to_recompute:
            ...
            elif spec1flux is not None and spec2flux is not None:
            ...
            if starType1=="DA" or starType1=="DC" or starType1=="DB" or starType1=="ELM":  model_wl1, model_spectrum_star1 = return_model_spectrum_DA(wl_all1_N, ref_wl, cut_wl_min, cut_wl_max, Grav1_N, flux1_N, Teff1_N, T1, logg1, Ts_Gs_unique=star1_Ts_Gs)
            ...
            if starType2=="DA" or starType2=="DC" or starType2=="DB" or starType2=="ELM":  model_wl2, model_spectrum_star2 = return_model_spectrum_DA(wl_all2_N, ref_wl, cut_wl_min, cut_wl_max, Grav2_N, flux2_N, Teff2_N, T2, logg2, Ts_Gs_unique=star2_Ts_Gs)
        ...
        for (ii, in_fi, sh_rv, modha, cut_lim, norm_lim, aHJD, sigclipspec) in zip(index[mask_ref_wl], input_files[mask_ref_wl], share_rv[mask_ref_wl], modelHa[mask_ref_wl], cut_Ha_all[mask_ref_wl], normaliseHa_all[mask_ref_wl], HJD_values[mask_ref_wl], sigma_clip[mask_ref_wl]):  # the mcmc RVs are gone through 1 by one, uses the ii counter.
            ...
            dlam2 = ref_wl_over_speed_of_light*mcmc_rv2
            ...
            if continuum_normalisation:
                ...
                try:
                    interparr = (R1*696340000)**2 / ((1000/mcmc_parallax)*3.085677581E16)**2  *interp(model_wl2, model_wl1+dlam1, model_spectrum_star1)    + (R2*696340000)**2 / ((1000/mcmc_parallax)*3.085677581E16)**2 * interp(model_wl2, model_wl2+dlam2, model_spectrum_star2)
                    interparr *= 4*np.pi
                except: 
                    interparr = (R1*696340000)**2 / ((1000/plx)*3.085677581E16)**2  *interp(model_wl2, model_wl1+dlam1, model_spectrum_star1)    + (R2*696340000)**2 / ((1000/plx)*3.085677581E16)**2 * interp(model_wl2, model_wl2+dlam2, model_spectrum_star2)
                    interparr *= 4*np.pi
            
            
            
            key = (ref_wl, inp_resolution, model_wl2[10], model_wl2[9])
            if key not in KERNEL_CACHE:
                kern_array = Gaussian1DKernel(stddev=0.5*(ref_wl/inp_resolution)/(model_wl2[10]-model_wl2[9])).array
            ...
            #interparr = convolve(interparr, KERNEL_CACHE[key], boundary = 'extend')
            #interparr = convolve1d(interparr, KERNEL_CACHE[key], mode="nearest")
            interparr = correlate1d(interparr, KERNEL_CACHE[key], mode="nearest") # this works for symmetric functions (gaussian = good) and is about 3% faster, but for non-symmetric use astropy convolve1d
            ...
            if continuum_normalisation:
                ...
                desired_range = (normalised_wavelength>ref_wl+modha[0])  &  (normalised_wavelength<ref_wl+modha[1])
            else:
                desired_range = normalised_wavelength>1200  # not supporting LyA fitting
            ...
            chisq_mask = desired_range
            chisq_indiv = -0.5*npsum((np_square(normalised_flux[chisq_mask][~clip_mask]-(off+interparr[chisq_mask][~clip_mask])))/np_square(normalised_err[chisq_mask][~clip_mask]))
            chisq_spec += chisq_indiv
            
            if not np.isfinite(chisq_indiv):
                thing = (np_square(normalised_flux[chisq_mask][~clip_mask]-(off+interparr[chisq_mask][~clip_mask])))/np_square(normalised_err[chisq_mask][~clip_mask])
                plt.plot(normalised_wavelength[chisq_mask][~clip_mask], normalised_flux[chisq_mask][~clip_mask])
                plt.plot(normalised_wavelength[chisq_mask][~clip_mask], off+interparr[chisq_mask][~clip_mask])
                #plt.title(str(np.amin(thing)) + "   " + str(np.amax(thing)) + "    " +  str(np.median(thing)))
                
                arg = np.argwhere((thing==np.nan) | (thing==np.inf) | (thing==-np.inf))
                aa, aa2 = normalised_wavelength[chisq_mask][~clip_mask][arg],thing[arg]
                raise ValueError(aa, normalised_flux[chisq_mask][~clip_mask][arg], (off+interparr[chisq_mask][~clip_mask])[arg])
                #plt.title(str())
                plt.show()
...
def lnlike_gauss_lorentz(theta, arguments):
    ...
    for ref_wl in the_unique_wavelengths:
        ...
        for (ii, in_fi, sh_rv, modha, cut_lim, norm_lim, aHJD, sigclipspec) in zip(index[mask_ref_wl], input_files[mask_ref_wl], share_rv[mask_ref_wl], modelHa[mask_ref_wl], cut_Ha_all[mask_ref_wl], normaliseHa_all[mask_ref_wl], HJD_values[mask_ref_wl], sigma_clip[mask_ref_wl]):  # the mcmc RVs are gone through 1 by one, uses the ii counter.
            ...
            # Smear to the resolution desired
            kern_array = Gaussian1DKernel(stddev=0.5*(ref_wl/inp_resolution)/(model_wl2[10]-model_wl2[9])).array
            kern_array /= kern_array.sum()
            interparr = convolve1d(interparr, kern_array, mode = 'nearest')
            interparr = interp(normalised_wavelength, model_wl2, interparr)
            ...
            ## Now on to calculating chisq
            chisq_indiv = -0.5*npsum((np_square(normalised_flux[desired_range][~clip_mask]-(off+interparr[desired_range][~clip_mask])))/np_square(normalised_err[desired_range][~clip_mask]))
            chisq_spec += chisq_indiv
...
def lnprior_quad_lorentz(theta, arguments):
    ...
    if not "run_double_one_by_oneMCMC" in sys_args:
        args = npargwhere(p0labels=="A1_1")[0][0];   gaussLorentz1_A1 = theta[args]
        args = npargwhere(p0labels=="A2_1")[0][0];   gaussLorentz2_A1 = theta[args]
        args = npargwhere(p0labels=="sig1_1")[0][0];   gaussLorentz1_sigma1 = theta[args]
        args = npargwhere(p0labels=="sig2_1")[0][0];   gaussLorentz2_sigma1 = theta[args]
...
if sys_args[1]=="RV" or sys_args[1]=="RV_gauss" or sys_args[1]=="RV_double_commonRV12":
    desired_wl=6562.81
    ...
    if starType1.startswith("D") or starType2.startswith("D") or starType1=="ELM" or starType2=="ELM":
        ...
        for tempcnt, (temperature_star, logg_star, starType) in enumerate(zip([T1_med,T2_med],[logg1_med,logg2_med], [starType1,starType2])):
            ...
            if starType=="DA":
                ...
                if tempcnt==1:   Grav2_N, wl_all2_N, flux2_N, Teff2_N = return_ELMgrids(temperature_star, logg_star)
    ...
    if starType1.startswith("D") or starType2.startswith("D"):
        if starType1=="DA" or starType1=="DC" or starType1=="DB" or starType1=="ELM": model_wl1, model_spectrum_star1 = return_model_spectrum_DA(wl_all1_N, desired_wl, cut_limits_min-20, cut_limits_max+20, Grav1_N, flux1_N, Teff1_N, T1_med, logg1_med)
        ...
        if starType2=="DA" or starType2=="DC" or starType2=="DB" or starType2=="ELM": model_wl2, model_spectrum_star2 = return_model_spectrum_DA(wl_all2_N, desired_wl, cut_limits_min-20, cut_limits_max+20, Grav2_N, flux2_N, Teff2_N, T2_med, logg2_med)
    ...
    if sys_args[1]=="RV_gauss":
        ...
        #sampler = EnsembleSampler(nwalkers, ndim, lnprob_gauss, pool=pool, args=[[input_files, reference_wl, cut_Ha_all, normaliseHa_all, list_norm_wl_grids, list_normalised_flux, list_normalised_err,  resolutions, used_RV_boundaries, model_wl1, model_spectrum_star1, model_wl2, model_spectrum_star2, desired_refwl]])
        ...
        #if plot_corner==True:
        #    fig2 = corner(samples, labels=p0labels, range=p0range, bins=30,smooth=True, quantiles=[0.16, 0.5, 0.84], show_titles=True, labels_args={"fontsize": 40}, title_fmt = '.3f')
    ...
    if sys_args[1]=="RV_double_commonRV12":  commonRV_1_2=True
    ...
    for ii, (in_fi, modha, cut_lim, norm_lim, aHJD, sigclipspec) in enumerate(zip(input_files[mask_ref_wl], modelHa[mask_ref_wl], cut_Ha_all[mask_ref_wl], normaliseHa_all[mask_ref_wl], HJD_values[mask_ref_wl], sigma_clip[mask_ref_wl])):
        ...
        try:
            ...
            kern_array /= kern_array.sum()
            smear_model_spectrum_star1 = convolve1d(model_spectrum_star1, kern_array, mode = 'nearest')
            
            kern_array = Gaussian1DKernel(stddev=0.5*(ref_wl/inp_resolution)/(model_wl2[10]-model_wl2[9])).array
            kern_array /= kern_array.sum()
            smear_model_spectrum_star2 = convolve1d(model_spectrum_star2, kern_array, mode = 'nearest')
            ...
            else: RV1err=10; RV2err=10  #  guess that it'll be ~10kms-1
            ...
            if True:
                ...
                if sys_args[1]=="RV_double_commonRV12":
                    rvfilename.append([in_fi, aHJD, RV1, RV1err])
                    ax.set_title("V1=" + str(np.round(RV1,1)) + " " + str(np.round(RV1err,1)));  ax2.set_title("R=" + str(np.round(inp_resolution,2)) + "  " + str(rvbound1))
                else:
                    rvfilename.append([in_fi, aHJD, RV1, RV1err, RV2, RV2err])
                    ax.set_title("V1=" + str(np.round(RV1,1)) + " " + str(np.round(RV1err,1)) + "  V2=" + str(np.round(RV2,1)) + " " + str(np.round(RV2err,1)));  ax2.set_title("R=" + str(np.round(inp_resolution,2)) + "  " + str(rvbound1) + "|" + str(rvbound2))
    ...
    if starType1.startswith("D") or starType2.startswith("D") or starType1=="ELM":
        if forced_teff1!=0 and forced_logg1!=0 and forced_teff2!=0 and forced_logg2!=0 and starType1=="DA" and starType2=="DA":
            ...
            elif (starType1=="ELM" and starType2=="ELM"): 
                Grav1_N, wl_all1_N, flux1_N, Teff1_N = return_ELMgrids(forced_teff1, forced_logg1)
                Grav2_N, wl_all2_N, flux2_N, Teff2_N = return_ELMgrids(forced_teff2, forced_logg2)
            ...
            sampler = EnsembleSampler(nwalkers, ndim, lnprob, pool=pool, args=[[input_files, share_rv, reference_wl, cut_Ha_all, normaliseHa_all, list_norm_wl_grids, list_normalised_flux, list_normalised_err,  resolutions, used_RV_boundaries, HJD_values, sigma_clip, npunique(reference_wl), None, None, None, None, None]])
        ...
        sampler = EnsembleSampler(nwalkers, ndim, lnprob_gauss_lorentz, pool=pool, args=[[input_files, share_rv, reference_wl, cut_Ha_all, normaliseHa_all, list_norm_wl_grids, list_normalised_flux, list_normalised_err,  resolutions, used_RV_boundaries, HJD_values, sigma_clip, npunique(reference_wl), None, None, None, None, None]])
    ...
    if not "run_double_one_by_oneMCMC" in sys_args:
        ...
        if plot_burnin:
            fig = corner(samples, labels=p0labels, range=p0range, bins=30,smooth=True, quantiles=[0.16, 0.5, 0.84], show_titles=True, labels_args={"fontsize": 40}, title_fmt = '.3f')
    ...
    #    
    #        fig3 = corner(samples.T[ll*2:ll*2+2].T, labels=["RV1", "RV2"], range=[arvBounds1, arvBounds2], bins=30, smooth=True, quantiles=[0.16, 0.5, 0.84], show_titles=True, labels_args={"fontsize": 40}, title_fmt = '.3f')
    ...
    if "run_double_one_by_oneMCMC" in sys_args:  
        with open("out/result"+str(int(sys_args[-1])+1)+".out", 'w') as output_file:
...
if sys_args[1]=="ATM" or sys_args[1]=="plotOnly":
    result = open(getcwd+"/out/result.out").readlines()
    allRV1s, allRV2s = [], []
    ...
    if "run_double_one_by_oneMCMC" in sys_args:
        filesout = os.listdir(getcwd+"/out")
        files_one_by_one = []
        for ff in filesout:
        ...
        res = "result"+str(int(sys_args[-1])+1)+".out"
        
        result = open(getcwd+"/out/"+res).readlines()
    ...
    for ii, (in_fi, sh_rv, ref_wl, modha, cut_lim, norm_lim, mask_out_Ha_min, mask_out_Ha_max, aHJD, inp_resolution, sigclipspec) in enumerate(zip(input_files, share_rv, reference_wl, modelHa, cut_Ha_all, normaliseHa_all, mask_out_Ha_min_all, mask_out_Ha_max_all, HJD_values, resolutions, sigma_clip)):
        if ((starType1.startswith("GG") or starType1.startswith("LL") or starType1.startswith("GL")) and sh_rv==-1) or (sh_rv==-1 and not ii in share_rv):
            ...
            if (starType1.startswith("D") and starType2.startswith("D")) or (starType1=="ELM" and starType2=="ELM"):
                if starType1=="DA" or starType1=="DC" or starType1=="DB" or starType1=="ELM": model_wl1, model_spectrum_star1 = return_model_spectrum_DA(wl_all1_N, ref_wl, cut_limits_min, cut_limits_max, Grav1_N, flux1_N, Teff1_N, T1_med, logg1_med)
                ...
                if starType2=="DA" or starType2=="DC" or starType2=="DB" or starType2=="ELM": model_wl2, model_spectrum_star2 = return_model_spectrum_DA(wl_all2_N, ref_wl, cut_limits_min, cut_limits_max, Grav2_N, flux2_N, Teff2_N, T2_med, logg2_med)
            ...
            if continuum_normalisation:   desired_range = (normalised_wavelength>ref_wl+modelHa_min)  &  (normalised_wavelength<ref_wl+modelHa_max)
            else: desired_range = normalised_wavelength==normalised_wavelength
            ...
            if continuum_normalisation:   interparr = Scaling_med*f1 + f2
            else:
                if starType1=="DA":     R1 = get_MTR(T1_med, logg=logg1_med, return_R_from_T_logg=True, loaded_Istrate=loaded_Istrate, loaded_CO=loaded_CO, loaded_Althaus=loaded_Althaus, force_CO_MTR=force_CO_MTR[0])
                elif starType1=="DBA" or starType1=="DB" or starType1=="DC":
                ...
                interparr_plot_star1 = (R1*696340000)**2 / ((1000/parallax_med)*3.085677581E16)**2  *interp(model_wl2, model_wl1+dlam1, model_spectrum_star1)
                interparr_plot_star2 = (R2*696340000)**2 / ((1000/parallax_med)*3.085677581E16)**2 * interp(model_wl2, model_wl2+dlam2, model_spectrum_star2)
                interparr = (R1*696340000)**2 / ((1000/parallax_med)*3.085677581E16)**2  *interp(model_wl2, model_wl1+dlam1, model_spectrum_star1)    + (R2*696340000)**2 / ((1000/parallax_med)*3.085677581E16)**2 * interp(model_wl2, model_wl2+dlam2, model_spectrum_star2)
                interparr *= 4*np.pi
                interparr_plot_star1 *= 4*np.pi
                interparr_plot_star2 *= 4*np.pi
            ...
            if continuum_normalisation==False:
                interparr_plot_star1 = convolve1d(interparr_plot_star1, kern_array, mode = 'nearest')
                interparr_plot_star2 = convolve1d(interparr_plot_star2, kern_array, mode = 'nearest')
                
                interparr_plot_star1 = interp(normalised_wavelength, model_wl2, interparr_plot_star1)
                interparr_plot_star2 = interp(normalised_wavelength, model_wl2, interparr_plot_star2)
            ...
            if continuum_normalisation:
                ...
                if input_vacuum_wl==True: # convert back into vacuum wavelengths for plotting if needed
                    normalised_wavelength = normalised_wavelength = list_norm_wl_grids[0]
                ...
                if starType1.startswith("D") or starType1=="ELM":
                ...
                if starType2.startswith("D") or starType2=="ELM":
                ...
                want_ergscm2AA=True
                if want_ergscm2AA:
                	FluxIn_Jy = 1E23 * normalised_flux
                	FluxIn_ergcm2AA = 2.99792458E-05 * FluxIn_Jy / np_square(normalised_wavelength)
                	normalised_flux = FluxIn_ergcm2AA
                	
                	interparr = 2.99792458E-05 * 1E23 * interparr / np_square(normalised_wavelength)
                	interparr_plot_star1 = 2.99792458E-05 * 1E23 * interparr_plot_star1 / np_square(normalised_wavelength)
                	interparr_plot_star2 = 2.99792458E-05 * 1E23 * interparr_plot_star2 / np_square(normalised_wavelength)
                
                
                if input_vacuum_wl==True: # convert back into vacuum wavelengths for plotting if needed
                    normalised_wavelength = list_norm_wl_grids[0]
                
                
                
                np.savetxt("out/forPlotting/fit_"+in_fi+"_"+str(ref_wl)+".dat", np.array([normalised_wavelength, interparr]).T)
                np.savetxt("out/forPlotting/data_for_fit_"+in_fi+"_"+str(ref_wl)+".dat", np.array([normalised_wavelength, normalised_flux]).T)
                
                exponent_for_plotting = int(np.floor(np.log10(np.amax(normalised_flux))))
                	
                plt.plot(normalised_wavelength, normalised_flux/10**exponent_for_plotting, c='grey', alpha=0.5, label="Data (raw)")
                
                boxcar_kernel = np.ones(5) / 5
                smoothed_data = np.convolve(normalised_flux, boxcar_kernel, mode='same')
                
                
                
                plt.plot(normalised_wavelength[5:-5], smoothed_data[5:-5]/10**exponent_for_plotting, c='k', label="Data (smoothened)")
                
                plt.plot(normalised_wavelength, interparr/10**exponent_for_plotting, c='r', label="Total")
                ax.set_title(filename)
                plt.plot(normalised_wavelength, interparr_plot_star1/10**exponent_for_plotting, c='b', ls='--', label="Star 1")
                plt.plot(normalised_wavelength, interparr_plot_star2/10**exponent_for_plotting, c='g', ls='--', label="Star 2")
                
                np.savetxt("out/forPlotting/fit_star1"+filename.split(".dat")[0]+".dat", np.array([normalised_wavelength, interparr_plot_star1]).T)
                np.savetxt("out/forPlotting/fit_star2"+filename.split(".dat")[0]+".dat", np.array([normalised_wavelength, interparr_plot_star2]).T)
                
                
                plot_wl_mask = normalised_wavelength>1200
                plt.ylim(0, np.amax(normalised_flux[desired_range & plot_wl_mask])/10**exponent_for_plotting + 0.25)
                plt.xlim(np.amin(normalised_wavelength[plot_wl_mask]), np.amax(normalised_wavelength[plot_wl_mask]))
                
                if sigclipspec!=-1:
                    resid = normalised_flux-interparr
                    resid_in_sig = resid/np.std(np.abs(resid[desired_range]))
                    clip_mask = (np.abs(resid_in_sig)>sigclipspec) & desired_range & ((normalised_wavelength > ref_wl+5) | (normalised_wavelength < ref_wl-5))
                    plt.scatter(normalised_wavelength[clip_mask], normalised_flux[clip_mask]/10**exponent_for_plotting, c='r', s=4)
                ...
                #if np.amin(normalised_wavelength)<2000:
                #	plt.plot(normalised_wavelength[all_masked], interparr[all_masked]/1E-26,c='r')
                
                
                plt.xlabel(r"Wavelength [\AA]")
                plt.ylabel(r"Flux [10$^{" +str(exponent_for_plotting) + "}$ erg/cm$^2$/s/Hz]")
                plt.legend()
                
                #plt.scatter(0,1,alpha=0, label="V1 = " + str(np.round(rv_med1,3)))
                #plt.scatter(0,1,alpha=0, label="V2 = " + str(np.round(rv_med2,3)))
                #plt.scatter(0,1,alpha=0, label="wl = " + str(ref_wl))
                #if starType1.startswith("D"):
                #    plt.scatter(0,1,alpha=0, label="T1 = " + str(np.round(T1_med,0)))
                #    plt.scatter(0,1,alpha=0, label="Logg1 = " + str(np.round(logg1_med,3)))
                #if starType2.startswith("D"):
                #    plt.scatter(0,1,alpha=0, label="T2 = " + str(np.round(T2_med,0)))
                #    plt.scatter(0,1,alpha=0, label="Logg2 = " + str(np.round(logg2_med,3)))
