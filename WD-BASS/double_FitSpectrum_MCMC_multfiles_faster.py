...
if starType1.startswith("D") or starType2.startswith("D"):
    ...
    if pier_or_antoine=="pier3Dphot_antoine1Dspec":
        ...
        unique_Teffs_logg_gteq_7, unique_Teffs_logg_lt_7, unique_Teffs_logg_6p5_7p0  =  npunique(Teff_all_logg_gteq_7), npunique(Teff_all_logg_lt_7), npunique(Teff_all_logg_6p5_7p0)
        ...
        else:                    wl_all_ant1D, flux_all_ant1D, Teff_all_ant1D, Grav_all_ant1D = load_models.load_models_DA_AntoineNLTE(minwl=npamin(reference_wl)-150,maxwl=npamax(reference_wl)+250);   mask_wl_5100_6200 = ((wl_all_ant1D<=6200) & (wl_all_ant1D>=5100));   wl_all_ant1D, flux_all_ant1D, Teff_all_ant1D, Grav_all_ant1D = wl_all_ant1D[~mask_wl_5100_6200], flux_all_ant1D[~mask_wl_5100_6200], Teff_all_ant1D[~mask_wl_5100_6200], Grav_all_ant1D[~mask_wl_5100_6200]
...
#for x,y in zip(HJD_values, RV_boundaries1):
#	print(x,y)
#raise ValueError
    ...
    if pier_or_antoine=="pier" or pier_or_antoine=="pier3Dphot_antoine1Dspec":
        if not (logg_star<7 and (temperature_star>14000 or temperature_star<5000)):   # 3D DA grid (unique structure for each of the below categories). Limits Logg = 5-9 . Teff 5000-14000
            ...
            if logg_star>6.5 and logg_star<=7.0:
                ...
                Teff_min=unique_Teffs_logg_6p5_7p0[npargwhere(temdiff==npamin(temdiff[temdiff>0]))[0][0]]
                Teff_max=unique_Teffs_logg_6p5_7p0[npargwhere(temdiff==npamax(temdiff[temdiff<0]))[0][0]]
                ...
                Teff_min=unique_Teffs_logg_lt_7[npargwhere(temdiff==npamin(temdiff[temdiff>0]))[0][0]]
                Teff_max=unique_Teffs_logg_lt_7[npargwhere(temdiff==npamax(temdiff[temdiff<0]))[0][0]]
                ...
                Teff_min=unique_Teffs_logg_gteq_7[npargwhere(temdiff==npamin(temdiff[temdiff>0]))[0][0]]
                Teff_max=unique_Teffs_logg_gteq_7[npargwhere(temdiff==npamax(temdiff[temdiff<0]))[0][0]]
            ...
            if pier_or_antoine=="pier":
                ...
                Teff_min=unique_Teffs_ant1D[npargwhere(temdiff==npamin(temdiff[temdiff>0]))[0][0]]
                Teff_max=unique_Teffs_ant1D[npargwhere(temdiff==npamax(temdiff[temdiff<0]))[0][0]]
                ...
                if len(npunique(antTeff1_N))==3:
                    un_teffs = npunique(antTeff1_N)
            ...
            Teff_min=unique_Teffs_1D[npargwhere(temdiff==npamin(temdiff[temdiff>0]))[0][0]]
            Teff_max=unique_Teffs_1D[npargwhere(temdiff==npamax(temdiff[temdiff<0]))[0][0]]
            ...
            #print(temperature_star, logg_star, npamin(Grav1_N), npamax(Grav1_N), npamin(Teff1_N), npamax(Teff1_N), npamin(wl_all1_N), npamax(wl_all1_N))
    ...
    for ref_wl in the_unique_wavelengths:
        ...
        for (ii, in_fi, sh_rv, modha, cut_lim, norm_lim, aHJD, sigclipspec) in zip(index[mask_ref_wl], input_files[mask_ref_wl], share_rv[mask_ref_wl], modelHa[mask_ref_wl], cut_Ha_all[mask_ref_wl], normaliseHa_all[mask_ref_wl], HJD_values[mask_ref_wl], sigma_clip[mask_ref_wl]):  # the mcmc RVs are gone through 1 by one, uses the ii counter.
            ...
            if sigclipspec!=-1:
                ...
                clip_mask = (np.abs(resid_in_sig[desired_range])>sigclipspec) & ((normalised_wavelength[desired_range] > ref_wl+4) | (normalised_wavelength[desired_range] < ref_wl-4))
...
if sys.argv[1]=="RV" or sys.argv[1]=="RV_gauss" or sys.argv[1]=="RV_double_commonRV12":
    ...
    for ii, (in_fi, modha, cut_lim, norm_lim, aHJD, sigclipspec) in enumerate(zip(input_files[mask_ref_wl], modelHa[mask_ref_wl], cut_Ha_all[mask_ref_wl], normaliseHa_all[mask_ref_wl], HJD_values[mask_ref_wl], sigma_clip[mask_ref_wl])):
        ...
        try:
            ...
            if starType1=="quadLorentz" and starType2=="quadLorentz":
                ...
                m4, m3, m2, m1, c = polyfit((normalised_wavelength-ref_wl), normalised_flux - interp(normalised_wavelength, model_wl1, smear_model_spectrum_star1_temp)  - interp(normalised_wavelength, model_wl2, smear_model_spectrum_star2_temp), 4)
                ...
                if False:
                    ...
                    for cnt, val in enumerate(combined_spectrum_dummy):
                        ...
                        except:   None #mask_negative_grad = np.append(mask_negative_grad, True)
                    ...
                    smear_model_spectrum_star2[mask_negative_grad] = npamax(smear_model_spectrum_star2[~mask_negative_grad])
            ...
            if Low_SNR:
                ...
                if sigclipspec!=-1:
                    ...
                    clip_mask = (np.abs(resid_in_sig)<sigclipspec) | ((normalised_wavelength < desired_refwl+4) & (normalised_wavelength > desired_refwl-4))
            ...
            if sigclipspec!=-1:
                ...
                mask_narrow = ((normalised_wavelength > desired_refwl-40) & (normalised_wavelength < desired_refwl+40)) | ((normalised_wavelength<desired_refwl+norm_limits_min+4) | (normalised_wavelength>desired_refwl+norm_limits_max-4))
                ...
                resid_in_sig = resid/np.std(np.abs(resid))
                clip_mask2 = ((np.abs(resid_in_sig)<sigclipspec) | ((normalised_wavelength[mask_narrow] < desired_refwl+4) & (normalised_wavelength[mask_narrow] > desired_refwl-4))) | ((normalised_wavelength[mask_narrow]>desired_refwl+norm_limits_max-4) | (normalised_wavelength[mask_narrow]<desired_refwl+norm_limits_min+4))
                
                if False: #starType1=="quadLorentz" and starType2=="quadLorentz":
                    m4, m3, m2, m1, c = polyfit((normalised_wavelength[mask_narrow][clip_mask2]-ref_wl), normalised_flux[mask_narrow][clip_mask2] - interp(normalised_wavelength[mask_narrow][clip_mask2], model_wl1+model_wl1*RV1/3E5, smear_model_spectrum_star1) - interp(normalised_wavelength[mask_narrow][clip_mask2], model_wl2+model_wl2*RV2/3E5, smear_model_spectrum_star2), 4)
                    quad = m4 * (model_wl1-ref_wl)**4  +  m3 * (model_wl1-ref_wl)**3  +  m2 * np_square(model_wl1-ref_wl)  +  m1*(model_wl1-ref_wl)  +  c
                    smear_model_spectrum_star1= smear_model_spectrum_star1_temp + 0.5*quad
                    smear_model_spectrum_star2 = smear_model_spectrum_star2_temp + 0.5*quad
                    
                    #plt.plot(model_wl1, smear_model_spectrum_star1);   plt.plot(model_wl2, smear_model_spectrum_star2);   plt.plot(model_wl1, quad);    plt.show()
    ...
    if "run_double_one_by_oneMCMC" in sys.argv:  spectrum_number = str(sys.argv[-1])
...
if sys.argv[1]=="ATM" or sys.argv[1]=="plotOnly":
    ...
    for ii, (in_fi, sh_rv, ref_wl, modha, cut_lim, norm_lim, mask_out_Ha_min, mask_out_Ha_max, aHJD, inp_resolution, sigclipspec) in enumerate(zip(input_files, share_rv, reference_wl, modelHa, cut_Ha_all, normaliseHa_all, mask_out_Ha_min_all, mask_out_Ha_max_all, HJD_values, resolutions, sigma_clip)):
        if ((starType1.startswith("GG") or starType1.startswith("LL") or starType1.startswith("GL")) and sh_rv==-1) or (sh_rv==-1 and not ii in share_rv):
            ...
            if sigclipspec!=-1:
                resid = normalised_flux-interparr
                resid_in_sig = resid/np.std(np.abs(resid[desired_range]))
                clip_mask = (np.abs(resid_in_sig)>sigclipspec) & desired_range & ((normalised_wavelength > ref_wl+4) | (normalised_wavelength < ref_wl-4))
    ...
    for unique_shared_rv in npunique(share_rv[where_share_rv]):
        ...
        if unique_shared_rv in plot_fit  and not starType1=="quadLorentz":   plt.show()
        plt.clf();   plt.close()
